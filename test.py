...
def main_worker(rank, world_size, args):
    ...
    reference_unet = DDP(reference_unet, device_ids=[rank])
    denoising_unet = DDP(denoising_unet, device_ids=[rank])
    pose_guider = DDP(pose_guider, device_ids=[rank])
    image_enc = DDP(image_enc, device_ids=[rank])

    pipe = Pose2VideoPipeline(
        vae=vae.module,
        image_encoder=image_enc.module,
        reference_unet=reference_unet.module,
        denoising_unet=denoising_unet.module,
        pose_guider=pose_guider.module,
        scheduler=scheduler,
    )
    pipe = pipe.to(rank, dtype=weight_dtype)
